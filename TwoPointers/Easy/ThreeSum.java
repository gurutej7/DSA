...
public class ThreeSum {
    ...
    public static List<List<Integer>> threeSumBetter(int [] nums){
        ...
        for(int i=0; i<nums.length-2;i++){
            ...
            while(l <r ){
                ...
                else if (sum >0) r--; // If current sum is grester than zero then decrease the larger value else vice versa
                else if (sum<0) l++;
            }
    ...
    public static List<List<Integer>> threeSum(int[] a) {
        ...
        for(int  i = 0 ; i< n-2 ; i++){
            //We can eliminate the usage of set by including some checks to avoid duplicates
            if (i > 0 && a[i] == a[i - 1]) 
            // skip same result
            continue;
            ...
            while(left < right){
                ...
                if(sum == 0){
                    ...
                    temp.add(a[right]);
                    ans.add(temp);
                    while(left < right && a[left]== temp.get(1))left++; //skip the same value at left
                    while(left < right && a[right] == temp.get(2))right--;//skip the same value at right
                }
                else if(sum > 0) right--;
                else left++;
